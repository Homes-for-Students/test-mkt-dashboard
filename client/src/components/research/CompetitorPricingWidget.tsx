  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h4 className="font-semibold text-slate-800 text-sm sm:text-base">Local Competitors</h4>
      </div>
      ...
      {data && !error && (
        <div className="space-y-3">
          <div className="grid grid-cols-12 text-[11px] text-slate-400 uppercase font-semibold tracking-wider pb-1 border-b border-slate-100 px-1">
            <span className="col-span-5 pl-2">Accommodation</span>
            <span className="col-span-4">Incentive</span>
            <span className="col-span-3 text-center">From Price</span>
          </div>
          <div className="max-h-[420px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
            {data.map((c: any) => (
              <div key={c.id} className="grid grid-cols-12 items-center text-sm py-2 border-b border-slate-50 last:border-0 hover:bg-slate-50 px-1 -mx-1 rounded transition-colors gap-2">
                <div className="col-span-5 truncate pl-2">
                  <div className="font-medium text-slate-800 truncate text-xs sm:text-sm" title={c.name}>{c.name}</div>
                  <div className="text-[10px] text-slate-400 truncate">{c.landlord || c.city}</div>
                </div>
                <div className="col-span-4 text-[11px] sm:text-xs text-left text-slate-600 font-medium break-words leading-tight whitespace-normal">
                  {c.incentive || '-'}
                </div>
                <div className="col-span-3 text-center font-semibold text-slate-700 text-xs sm:text-sm">
