...
- **Executive Analytics:** Dynamic performance overview tracking spends, clicks, conversions, and direct sheets revenue across channels.
- **Search Performance Hub:** Integrated organic Google Search Console metrics alongside Google Ads campaign tracking.
- **Market & City Insights:** Real-time, on-demand local intelligence matching property coordinates and city names:
  - **Local Competitor Pricing:** Aggregates and sorts current rates and incentive deals from StudentCrowd.
  - **Local Crime Statistics:** Hyper-local crime event counts and categories from `data.police.uk`.
  - **Transit Disruptions:** Live datasets and operators from the DfT Bus Open Data Service (BODS).
- **Brand Colors Manager:** Dynamic brand identity configuration and theme customization.
- **Secure Shared Dashboards:** Generates temporary, read-only dashboard links for external clients.
...
- `STUDENT_CROWD_API_KEY`: API Key for StudentCrowd queries
- `BODS_API_KEY`: API Key for DfT Bus Open Data Service
...
This application is ready for deployment in a modern containerized environment (e.g., Google Cloud Run, AWS ECS, or Docker Compose). Below are the details for setting up your CI/CD pipeline.

### Build Scripts
- **Build Client & Server**: Run `npm run build`. This runs:
  - `vite build`: Compiles the React SPA into `dist/client`.
  - `esbuild server/_core/index.ts`: Bundles the Node Express backend and tRPC router into `dist/index.js`.
- **Database Schema Sync**: Run `npm run db:push` in your release stage to generate and apply database schema updates using Drizzle.
...
|---|---|
| `DATABASE_URL` | MySQL connection string for the database (e.g., `mysql://user:pass@host:port/db`) |
| `STUDENT_CROWD_API_KEY` | API Key for StudentCrowd queries (market/pricing data) |
| `BODS_API_KEY` | API Key for DfT Bus Open Data Service (transit disruptions data) |
| `GOOGLE_MAPS_API_KEY` | Google Maps API key (if fallback geocoding is needed) |

### Docker Setup
A standard production Dockerfile can be created to build and serve the application:
...
You can configure a GitHub Actions workflow `.github/workflows/deploy.yml` to automatically build, check, and deploy:
```yaml
